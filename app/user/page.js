...
export default function Page() {
  ...
  const onSubmit = async () => {
    ...
    axios({
      ...
      .then(res => {
        ...
        Toast.fire({
          icon: 'success',
          title: `login ${res.data.Email} sebagai user`
        })
        router.replace('/')
        setIsLoading(false)
      })
  ...
  return (
    <section className="h-screen flex flex-col md:flex-row justify-center space-y-10 md:space-y-0 md:space-x-16 items-center my-2 mx-5 md:mx-0 md:my-0">
      ...
      <div className="md:w-1/3 max-w-sm">
        <input onChange={(e) => onChange(e, 'email')} className="text-sm w-full px-4 py-2 border text-[gray] border-solid border-gray-300 rounded" type="text" placeholder="Email Address" />
